...
// Function to get height of set of points 
int getHeight(Mat H,int ch, Point i, Point f);

// Object for using std:sort on the cv::DMatch class
...
int depth_height_calc(Mat leftImg, Mat rightImg) 
{
     ...
     * parameters of the camera. The left rectified image is stored
     * in img_left_remap and right rectified image is stored in 
     * img_right_remap.
     */
    ...
    /* In this part we refine the matches obtained in the above section
     * by sorting them according to distances and the taking a predefined
     * number of matches having the least Hessian Distance.
     * This reduces the number of matches significantly thus reducing the
     * time required
     */
	...
	for(i=0; i<good_matches.size();i++)
    {
        ...
        // and camera height
        height = getHeight(h,ch, Point2f(keypoints_1[good_matches[i].queryIdx ].pt.x,
...
/* getHeight function is used to get heights of different points in the image.
 * The parameters passed are 
 * 1. The homography matrix of ground plane. 
 * 2. The camera height. 
 * 3. Point in img_left_remap for good_matches[i].
 * 4. Point in img_right_remap for good_matches[i]

 * Algorithm: We have the ground plane homography which converts all points on 
 * ground in img_left_remap to the respective points in img_right_remap.
 * But as it is for ground plane, if this matrix is applied to any point
 * not on this plane, we wont get the point in img_right_remap as a result. 

 * For every point passed as an argument, homography matrix of all planes
 * parallel to the ground is computed at an interval of 1cm.
 * The point passed as an argument is multipled with all the matrices
 * on by one and the resultant is stored in (ex,ey). 
 * This is the expected matching point in img_right_remap of the point
 * in img_left_remap.

 * After getting all these (ex,ey), the one closest to the actual point in
 * img_right_remap is selected. The particular h for which we got the (ex,ey)
 * is the height in cm for that point.

 * The formula to calculated homography matrix of a plane at a distance d from
 * the camera, given the homography matrices of 2 planes at distances d1 and d2 
 * from the camera being h1 and h2. The formula can be checked out at this link.
 * http://www.google.co.in/url?sa=t&rct=j&q=&esrc=s&source=web&cd=9&ved=0CIIBEBYwCA&url=http%3A%2F%2Fsyedurrahman.tripod.com%2Fobstacle_detection.pdf&ei=zIN-UfDFK4PkrAeSz4H4CA&usg=AFQjCNHJ_DutJgjsSjcEp6uCz3PCK0TA8A&sig2=qkTwGn35BPwQdCeIPYFRhQ&bvm=bv.45645796,d.bmk&cad=rja
 */
...
int getHeight(Mat H, int ch, Point i, Point f)
{
    ...
    //	newH = H.mul(float(ch)/float(h-ch));
        ...
        // B will be calculated by multiplying A with newH
...
// returns height to main function
return (height);
}
